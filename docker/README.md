...
You can access your application via **`localhost`**, if you're running the containers directly, or through **`192.168.33.5`** when run on a vm. nginx and mailhog both respond to any hostname, in case you want to add your own hostname on your `/etc/hosts` 

Service|Address outside containers|Address outside VM
------|---------|-----------
Webserver|[test.local:8081](http://test.local:8081)
...
------|---------|-----------
php-fpm|test-php-fpm|9000
Postgres|test-postgres|5432 (default)
...
**Note 2:** if you're using Vagrant, you'll need to ssh into it first
  ...
  * Execute command inside of container: `docker exec -it test-php-fpm COMMAND` where `COMMAND` is whatever you want to run. For instance, `/bin/bash` to open a console prompt.
sudo docker exec -it test-webserver /bin/bash


To restore postgres container

docker exec -it $(docker-compose ps -q postgres9 ) pg_dump -Upostgres > backup.sql
sudo docker-compose rm test-postgres
sudo docker volume rm  phpdocker_postgres-db-volume
sudo docker-compose up --build  test-postgres
...
* do alias of your main ip to 10.254.254.254 `sudo ip addr add 10.254.254.254/24 brd + dev eth0 label eth0:1`
* Configurate xdebug with instruction from the article http://blog.arroyolabs.com/2016/10/docker-xdebug/
* Set debug proxy settings https://cloud.githubusercontent.com/assets/6569136/18808374/a79357a2-8260-11e6-8b40-c5165554aaf9.png